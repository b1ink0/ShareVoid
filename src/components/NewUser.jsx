...
export default function NewUser({ setUpdate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      console.log(
        "Username must be between 3 and 15 characters, must not contain special characters and should not start with a number!"
      );
      startTransition(() => {
        setWarning(
          "Username must be between 3 and 15 characters in lowercase, must not contain special characters and should not start with a number!"
        );
        setIsWarning(true);
      });
    }
  ...
  const handleRSAKey = async () => {
    let key = await handleGenerateKey();
    console.log(key);
    setKey(key);
  ...
  return (
    <>
      ...
      <form
        ...
        className="flex flex-col w-11/12 md:w-96 p-3 mt-7 border-4 border-[color:var(--bg-secondary)] rounded-lg"
      >
        ...
        <div className="input_container w-full mt-1 relative flex justify-center items-center">
          <input
            ...
            className="w-full bg-[color:var(--bg-secondary)] h-10 rounded-full outline-none pl-3 pr-3"
            name="username"
            id="username"
            required
          />
        ...
        {isWarning && (
          <p className="w-full bg-[color:var(--bg-secondary)] p-3 rounded-lg text-justify text-red-500 mt-3">
            {warning}
          </p>
        )}
        <button
          className="w-full mt-3 mb-2 pt-0 pb-1 pr-2 pl-2 rounded-full bg-blue-700"
          type="button"
          disabled={rsaKeyLoading ? true : false}
          onClick={() => {
            setRsaKeyLoading(true);
            handleRSAKey();
          }}
        >
          {rsaKeyLoading ? "Generating Key..." : "Generate RSA Key Pair"}
        </button>
        {loading ? (
          <p>Loading...</p>
        ) : (
          ...
          key.privateKey && (
            <>
              <label htmlFor="publicKey" className="mt-2 font-bold">
                Public Key:
              </label>
              <div className="textarea_container relative flex justify-center items-center mt-1">
                <textarea
                  className="w-full bg-[color:var(--bg-secondary)] h-36 rounded-lg outline-none mb-2 pl-3 pr-10 resize-none"
                  name="publicKey"
                  id="publicKey"
                  value={key.publicKey}
                  readOnly
              ...
              <label htmlFor="privateKey" className="mt-2 font-bold">
                Private Key:
              </label>
              <div className="textarea_container relative flex justify-center items-center mt-1">
                <textarea
              ...
              {!keyDownloaded ? (
                <div
                  className="w-full mb-2 mt-2 pt-0 pb-1 pr-2 pl-2 rounded-full bg-blue-700 text-center"
                  onClick={() => {
                    handleDownloadJSON(key.privateKey, "Private Key");
                    setKeyDownloaded(true);
                  }}
                >
                  Download Private Pair
                </div>
              ) : (
                <button
                  className="w-full mb-2 mt-2 pt-0 pb-1 pr-2 pl-2 rounded-full bg-green-600"
                  type="submit"
                >
                  Create Account
                </button>
