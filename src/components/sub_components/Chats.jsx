...
export default function Chats({
  ...
  return (
    <div
      ...
      className="w-full h-[calc(100%_-_40px)] md:h-full hidden md:flex"
    >
      <button
        onClick={() => setSearchUser(true)}
        className="md:hidden w-14 h-14 flex justify-center items-center bg-[color:var(--bg-secondary)] absolute bottom-3 z-10 right-3 rounded-full"
      >
        <div className="w-7 h-7 rotate-45">
          <CloseIcon />
        </div>
      </button>
      <div className="chats_container w-full md:w-72 h-full md:h-full pb-3 overflow-auto flex flex-col  justify-start items-center relative ">
        <button
          onClick={() => setSearchUser(true)}
          className="hidden md:flex w-14 h-14 justify-center items-center bg-[color:var(--bg-secondary)] absolute bottom-3 right-3 rounded-full"
        >
          <div className="w-7 h-7 rotate-45">
            <CloseIcon />
          </div>
        ...
        <div className="w-full h-full md:h-[calc(100%_-_40px)] overflow-auto px-2">
          {chats &&
            chats.map((chat, i) => (
              <div
              ...
              >
                ...
                <div className="chat_container flex flex-col justify-center items-start ml-3">
                  ...
                  {chat.latestMessage ? (
                    ...
                    chat.latestMessage.file.file_name ? (
                      <div className="w-40 flex justify-center items-center">
                        <div className="w-3 h-3">
                          <ClipIcon size={2} />
                        </div>
                        <p className="truncate w-full text-third">
                          {chat.latestMessage?.file?.file_name}
                        </p>
                      </div>
                    ) : chat.latestMessage.text &&
                      ...
                      <p className="truncate w-40 text-third">
                        {chat.latestMessage.text}
                      </p>
                    )
                  ) : (
      ...
      {currentChat && Object.keys(currentChat).length !== 0 && isDesktop && (
        <div className="w-[calc(100%_-_288px)] relative h-full flex flex-col border-l-2 border-l-[color:var(--bg-secondary)]">
          <Main
            key={Math.random()}
