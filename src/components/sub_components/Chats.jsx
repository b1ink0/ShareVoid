...
export default function Chats({ chats, setSearchUser, setCurrentChat, md, isDesktop, currentChat, currentUser, loggedIn, setProfile }) {
    return (
        <div style={{ display: md ? "none" : "flex" }} className="w-full h-[calc(100%_-_40px)] md:h-full hidden md:flex">
            <button onClick={() => setSearchUser(true)} className="md:hidden w-14 h-14 flex justify-center items-center bg-[color:var(--bg-secondary)] absolute bottom-3 z-10 right-3 rounded-full">
                ...
                <div className="w-full h-full md:h-[calc(100%_-_40px)] overflow-auto px-2">
                    {
                        chats && chats.map((chat, i) => (
                            <div onClick={() => setCurrentChat(chat)} key={nanoid()} className="chat_container w-full mt-3 p-2 rounded-lg h-20  bg-[color:var(--bg-secondary)] flex justify-start items-center ">
            ...
            }
            {
                ...
                <div className="w-[calc(100%_-_288px)] h-full flex flex-col border-l-2 border-l-[color:var(--bg-secondary)]">
