...
export default function SearchUser({ setSearchUser, setCurrentChat }) {
  ...
  const getUser = async (username) => {
    ...
    getDocs(publicUserRef).then((result) => {
      ...
      result.forEach((doc) => {
        console.log(doc.data(), result.size, localUids.includes(doc.data().uid),)
        users.push({ ...doc.data(), isLocal: localUids.includes(doc.data().uid) })
        if (result.size - 1 === i) {
          console.log(users)
          setSearching(false)
          setResults(users)
    ...
    <div className="fadeIn absolute w-full h-full flex justify-center items-center bg-[color:var(--bg-primary-semi-transparent)] z-20">
      <div className="w-10/12 h-2/3 p-3 bg-[color:var(--bg-secondary)] rounded-lg relative">
        ...
        <div className="search_results_container h-[calc(100%_-_52px)] overflow-auto bg-[color:var(--bg-primary)] rounded-lg p-3">
          {
            (!searching && results[0] === undefined && noResult) ? <p className="text-center h-40 flex justify-center items-center">User not found! <br /> ＞﹏＜</p> :
              (!searching && results[0] === undefined) ? <p className="text-center">Enter Username</p> :
                (searching && results[0] === undefined) ? <Skeleton count={7} style={{ width: "100%", height: "48px", marginBottom: "12px" }} />
                  :
                    ...
                    <div key={i} className="fadeIn search_result w-full h-12 flex justify-start pr-2 pl-2 items-center bg-[color:var(--bg-secondary)] rounded-lg mb-3 relative">
                      ...
                      <h3>{result.username}</h3>
                      {
                        result.uid !== currentUser.uid && !result.isLocal ?
                          <button onClick={() => { setAreYouSure(true); setSelectedUser(result) }} className="absolute right-4 w-9 h-9 rounded-full flex justify-center items-center">
                            <div className="w-4 h-4 rotate-45">
                              <CloseIcon />
                            </div>
                          </button>
                          :
                          <button onClick={() => handleSelectUser(result)} className="absolute right-4 w-9 h-9 rounded-full flex justify-center items-center">
